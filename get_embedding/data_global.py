# -*- coding: utf-8 -*-
"""
Created on Tue Nov  3 02:26:29 2020
...
def get_train_pos(interaction_path, gt_path):
    ...
    negative_item = [[] for _ in range(num_user)]
    for uid in range(num_user):
        if not train_pos[uid] == []:
            train_pos[uid] = np.array(train_pos[uid])
            negative_item[uid] = np.setdiff1d(np.arange(num_item), train_pos[uid])
            

    return train_pos, negative_item, num_user, num_item
